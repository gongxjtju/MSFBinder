...
def yizhe(feature_188D, label_188D, feature_localdpp, label_localdpp, feature_acstruct, label_acstruct,
    ...
    pred_localdpp1 = pred_localdpp[:, 1]
    model_localdpprf = RandomForestClassifier(n_estimators=290,random_state=0)
    model_localdpprf.fit(x_train_localdpp, y_train_localdpp)
    pred_localdpprf = model_localdpprf.predict_proba(x_test_localdpp)
    pred_localdppfr1 = pred_localdpprf[:, 1]
    model_localdpppusu = MultinomialNB(fit_prior=True)
    model_localdpppusu.fit(x_train_localdpp, y_train_localdpp)
    pred_localdpppusu = model_localdpppusu.predict_proba(x_test_localdpp)
    pred_localdpppusu1 = pred_localdpppusu[:, 1]
    ...
    model_188Drf = RandomForestClassifier(random_state=0,n_estimators=490)
    model_188Drf.fit(x_train_188D, y_train_188D)
    pred_188Drf = model_188Drf.predict_proba(x_test_188D)
    pred_188Drf1 = pred_188Drf[:, 1]
    model_188Dpusu = MultinomialNB(fit_prior=True)
    model_188Dpusu.fit(x_train_188D, y_train_188D)
    pred_188Dpusu = model_188Dpusu.predict_proba(x_test_188D)
    pred_188Dpusu1 = pred_188Dpusu[:, 1]
    ...
    model_dwtrf = RandomForestClassifier(n_estimators=660,random_state=0)
    model_dwtrf.fit(x_train_dwt, y_train_dwt)
    pred_dwtrf = model_dwtrf.predict_proba(x_test_dwt)
    pred_dwtrf1 = pred_dwtrf[:, 1]
    model_dwtpusu = MultinomialNB(fit_prior=True)
    model_dwtpusu.fit(x_train_dwt, y_train_dwt)
    pred_dwtpusu = model_dwtpusu.predict_proba(x_test_dwt)
    pred_dwtpusu1 = pred_dwtpusu[:, 1]
    ...
    model_acstructrf = RandomForestClassifier(n_estimators=190,random_state=0)
    ...
    model_acstructpusu = MultinomialNB(fit_prior=True)
    model_acstructpusu.fit(x_train_acstruct, y_train_acstruct)
    pred_acstructpusu = model_acstructpusu.predict_proba(x_test_acstruct)
    pred_acstructpusu1 = pred_acstructpusu[:, 1]
    pred = np.vstack((pred_localdpp1,pred_localdppfr1, pred_localdpppusu1,pred_188D1,pred_188Drf1, pred_188Dpusu1,pred_dwt1,pred_dwtrf1,pred_dwtpusu1,pred_acstruct1,pred_acstructrf1,pred_acstructpusu1)).T
    print pred.shape
    ...
    for i in range(5):
        ...
        y_pred_localdpppusu.extend(yizhetwolayerpusu(x_train_localdpp, y_train_localdpp, i, testspilt))
        ...
        y_pred_188Dpusu.extend(yizhetwolayerpusu(x_train_188D, y_train_188D, i, testspilt))
        ...
        y_pred_acstructpusu.extend(yizhetwolayerpusu(x_train_acstruct, y_train_acstruct, i, testspilt))
        ...
        y_pred_dwtpusu.extend(yizhetwolayerpusu(x_train_dwt, y_train_dwt, i, testspilt))
    pred_zong = np.vstack((y_pred_localdpp,y_pred_localdpprf,y_pred_localdpppusu, y_pred_188D,y_pred_188Drf, y_pred_188Dpusu,y_pred_dwt,y_pred_dwtrf,y_pred_dwtpusu, y_pred_acstruct,y_pred_acstructrf,y_pred_acstructpusu)).T
