...
def transtofeature(countpositive,countnege,lamudamax,n):
    ...
    for i in range(countpositive):
        print i
        line=readfile('F:/DNAbindingprotein/PDB186_seqs_PSSM/P_PSSM/'+str(i)+'.pssm')
        feature=local_DPP_1221gai(line,n,lamudamax)
        featurepositive.append(feature)
    featurenegative=[]
    for i in range(countnege):
        print i
        linene=readfile('F:/DNAbindingprotein/PDB186_seqs_PSSM/N_PSSM/'+str(i)+'.pssm')
    ...
    np.savetxt('F:/DNAbindingprotein/g_feature_gai1221_local_'+str(n)+'_'+str(lamudamax)+'_'+str(countnege+countpositive)+'.csv', feature, delimiter=',')
    label=[]
    label.extend(labelnegtive)
    label.extend(labelnpositive)
    np.savetxt('F:/DNAbindingprotein/g_label_gai1221_local_'+str(n)+'_'+str(lamudamax)+'_'+str(countnege+countpositive)+'.csv', label, delimiter=',')
#transtofeature()
import sys
